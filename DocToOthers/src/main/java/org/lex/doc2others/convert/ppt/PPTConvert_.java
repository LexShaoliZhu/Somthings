...
public class PPTConvert_ {
    private static Logger log = LoggerFactory.getLogger(PPTConvert_.class);
     ...
     * @param isVisble
     */
    public PPTConvert_(boolean isVisble) {
        if (null == ppt) {
    ...
    public PPTConvert_(String filePath, boolean isVisble) throws Exception {
        ...
        ActiveXComponent presentations = ppt.getPropertyAsComponent("Presentations");
        presentation = presentations.invokeGetComponent("Open", new Variant(fileAbPath), new Variant(true));
    }
    ...
    public void PPTToJPG(String pptfile, String saveToFolder) {
        try {
            saveAs(presentation, saveToFolder, PPT_SAVEAS_JPG);
...
//            }
     ...
     * @author YHY
     */
    public void saveAs(Dispatch presentation, String saveTo, int pptSaveAsFileType) throws Exception {
    ...
    public Dispatch addChart(Dispatch pageShapes, int chartType, int leftDistance, int topDistance, int width,
        ...
        Variant chart = Dispatch.invoke(pageShapes, ADD_CHART, 1,
                new Object[] { new Integer(chartType), // 图表类型
    ...
    public Dispatch addTable(Dispatch pageShapes, long rows, long columns, int leftDistance, int topDistance, int width,
            int height) throws Exception {
        return Dispatch
                .invoke(pageShapes, "AddTable", 1,
                        new Object[] { new Long(rows), new Long(columns), new Integer(leftDistance),
                                new Integer(topDistance), new Integer(width), new Integer(height) },
                        new int[1])
                .toDispatch();
    }
    ...
    public Dispatch getCellOfTable(Dispatch tableObj, int rowNum, int columnRum) throws Exception {
        return Dispatch.invoke(tableObj, "Cell", Dispatch.Method,
                new Object[] { new Long(rowNum), new Long(columnRum) }, new int[1]).toDispatch();
    }
    ...
    public void getGeneragePpt() throws Exception {
        ...
        Dispatch cell = Dispatch.invoke(workSheetItem, "Range", Dispatch.Get, new Object[] { "B3" }, new int[1])
                .toDispatch();
         ...
         * Object[]{"{5C22544A-7EE6-4342-B048-85BDC9FD1C3A}"}, new int[1]);
         * //获取单元格 Dispatch cell = getCellOfTable(tableObj, 1, 1); Dispatch
         * cell2 = getCellOfTable(tableObj, 2, 1); addCellValue(cell, "测试着玩的");
    ...
    public void invokePPTTemplate(String filePath) throws Exception {
         ...
         * Dispatch workBook=Dispatch.get(chartData, "Workbook").getDispatch();
         * 
         * Dispatch workSheets=Dispatch.get(workBook,
         * "Worksheets").getDispatch();
         * 
         * Dispatch workSheetItem = Dispatch.call(workSheets, "Item", new
         * Variant(1)).toDispatch();
         * 
         * Dispatch cell = Dispatch.invoke(shape1, "Range", Dispatch.Get, new
         * Object[] { "B3" }, new int[1]).toDispatch();
         */
        // 修改单个系列的图表类型
        // Dispatch Seri1 = Dispatch.call(chartObj, "SeriesCollection", new
        // Variant(1)).toDispatch();
        ...
        // Dispatch chartArea = Dispatch.get(chartObj,
        // "ChartArea").toDispatch();
        // 清楚图表内容
    ...
    // // PPTConvert jac = new PPTConvert(true);
    // PPTConvert jac = new PPTConvert(filePath, true);
    // // jac.getGeneragePpt();
